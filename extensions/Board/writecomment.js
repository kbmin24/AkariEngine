...
module.exports = async (req, res, boards, posts, boardcomment, block, perm) =>
{
    ...
    if (!articleNow)
    {
        require(paths.resolve('error.js'))(req, res, { description: '존재하지 않는 게시물입니다.', returnLink: '/board', returnName: '게시판 홈', statusCode: 404 })
        return
    }
    if (!req.session.username)
    {
        ...
        {
            require(paths.resolve('error.js'))(req, res, { description: '닉네임이 필요합니다.', returnLink: 'javascript:window.history.back()', returnName: '글쓰기', statusCode: 200 })
            return
        }
        if (req.body.pw.trim() == "")
        {
            require(paths.resolve('error.js'))(req, res, { description: '비밀번호가 필요합니다.', returnLink: 'javascript:window.history.back()', returnName: '글쓰기', statusCode: 200 })
            return
        }
    }
    if (!req.body.content)
    {
        require(paths.resolve('error.js'))(req, res, { description: '내용이 필요합니다.', returnLink: 'javascript:window.history.back()', returnName: '글쓰기', statusCode: 200 })
        return
    }
    ...
    {
        require(paths.resolve('error.js'))(req, res, { description: '잘못된 접근입니다.', returnLink: '/board', returnName: '게시판 홈', statusCode: 403 })
        return
    }
    ...
    {
        require(paths.resolve('error.js'))(req, res, { description: '이 게시판의 쓰기 권한이' + acl + ' 이기 때문에 댓글 작성이 불가합니다.', returnLink: 'javascript:window.history.back()', returnName: '이전 페이지', statusCode: 200 })
        return
    }
