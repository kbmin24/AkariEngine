        ...
        case 'changePassword':
            {
                ...
                if (!user)
                {
                    require(paths.resolve('error.js'))(req, res, null, `로그인이 필요합니다.`, '/login', '로그인 페이지', 403, 'ko')
                    return
                }
                ...
                crypto.pbkdf2(oldPassword, user.salt, 10000, 64, 'sha512', async (err, hashedPW) =>
                {
                    ...
                    {
                        //bad.
                        require(paths.resolve('error.js'))(req, res, null, `이전 비밀번호를 올바르게 입력했는지 확인해 주세요.`, 'javascript:window.history.back()', '이전 페이지', 403, 'ko')
                        return
                    }
